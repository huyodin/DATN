...
@section('content')
    <div class="container-lg mt-5 bg-white p-4">
        <h1 class="text-center text-primary mb-4">Chỉnh sửa Tour</h1>

        <form id="form-ai-store" action="{{ route('admin.tour.update', ['tour' => $tour->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="d-flex align-items-center justify-content-between">
                <a href="{{ route('admin.tour.index') }}" class="btn btn-primary" id="btn-back-ai"><i class="fas fa-long-arrow-alt-left"></i>&nbsp; Trở lại</a>
                <button type="submit" class="btn btn-success" id="btn-save-ai">Chỉnh sửa</button>
            </div>
            ...
            <div class="form-group pt-3">
                ...
                <div class="">
                    <input type="text" name="name" id="name-tour" class="form-control px-3" placeholder="Nhập tên tour" value="{{ $tour->name }}">
                </div>
            </div>

            <div class="form-group">
                <label for="description-tour" class="control-label text-muted mb-0">Nội dung tour</label>
                <textarea name="description" class="form-control js-count-limit px-3" id="description-tour" data-limit="200" rows="5">{{ $tour->description }}</textarea>
            </div>
            <div class="form-group">
            ...
            <div id="date-ranges-section">
                @foreach($tour->tourDates as $index => $date)
                    <div class="form-group d-flex align-items-center mb-3" id="date-range-{{ $index }}">
                        ...
                        <a class="btn-delete-date-range text-danger fs-26 ml-2" style="cursor:pointer;">&times;</a>
                    </div>
                @endforeach
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" name="price" class="form-control px-3" value="{{ $tour->price }}">
            </div>
            <div class="form-group">
                <label for="number_of_participants" class="control-label text-muted mb-0">Số lượng người</label>
                <input type="number" name="number_of_participants" class="form-control px-3" value="{{ $tour->number_of_participants }}">
            </div>
            ...
            <div>
                <div class="d-flex align-items-center justify-content-between">
                    <h3>Ảnh tour</h3>
                    <a class="btn-add-image fs-16 mb-3 btn btn-warning" id="btn-add-img-tour">＋Thêm ảnh</a>
                </div>
            ...
            $("#btn-add-img-tour").on("click", function () {
                ...
                <div class="col list-unstyled d-flex align-items-center flex-wrap mb-3 gap-3 div-ai-image">
                    <div class="rounded-3 me-2 no-drag-title" style="
                        ...
                        position: relative;
                        overflow: hidden;">
                        <img id="image-preview-${uniqueIndex}" src="https://placehold.co/400" alt="" style="
                        position: absolute;
                        top: 50%;
                        left: 50%;
                        width: 130%;
